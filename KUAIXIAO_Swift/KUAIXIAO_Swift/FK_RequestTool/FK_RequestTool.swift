...
extension FK_RequestTool{
    ...
    func getRequest(urlString:String, params :[String:Any],success: @escaping(_ response:[String :AnyObject])->(),failture:@escaping(_ error:Error)->()){
        ...
        var  param  = params
        param["appId"] = APPID as NSObject
        param["deviceName"] = FK_UIDeviceExtension.current.modelName as NSObject
        param["timeStamp"] = TIMESTAMP as NSObject
        param["imei"] = "888888" as NSObject
        param["version"] = "1.0.0" as NSObject
        let urlStr = "\(FKURL)\(urlString)/"

        Alamofire.request(urlStr, method:.get, parameters:param).responseJSON { (response) in
